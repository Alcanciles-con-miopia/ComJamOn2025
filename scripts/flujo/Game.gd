...
func getJson() -> void:
	for i in JsonParser.json_data["Mapas"][1]["Init"]:
		Global.initialCells.append(Vector2(i["x"], i["y"]))
	for i in JsonParser.json_data["Mapas"][1]["Expansion1"]:
		Global.expansion1.append(Vector2(i["x"], i["y"]))
	for i in JsonParser.json_data["Mapas"][1]["Expansion2"]:
		Global.expansion2.append(Vector2(i["x"], i["y"]))
	for i in JsonParser.json_data["Mapas"][1]["Expansion3"]:
		Global.expansion3.append(Vector2(i["x"], i["y"]))
	for i in JsonParser.json_data["Mapas"][1]["Expansion4"]:
